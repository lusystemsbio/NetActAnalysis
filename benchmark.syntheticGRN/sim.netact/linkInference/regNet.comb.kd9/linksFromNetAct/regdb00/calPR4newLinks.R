...
regdb.per <- readRDS(file = '../../../../../regDBs/regDB.nopert.rds')
...
stats.df <- calPR4newLinks(inferredRel.df=inferredRel.df, gtLinks.df <- regNetNOPonly, regNet.pert=regNet.pert, miTSH.list=miTSH.list)
...
newlinkCount<- stats.df$newlinks
plot(newlinkCount,
     lty=1, pch=19,
     #ylim = c(0, 1), 
     xlab = 'mutual information threshold', ylab = 'count',
     xaxt = "n")
axis(1,                         # Define x-axis manually
     at = 1:length(miTSH.list),
     labels = miTSH.list)
lines(newlinkCount, lty=2)
