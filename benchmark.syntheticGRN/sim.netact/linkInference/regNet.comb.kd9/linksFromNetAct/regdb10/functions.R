...
# Functions to calculate stats for new links: calPR4newLinks.R
#=============================================================
...
# Find the links in the inferred link set that are not part of the perturbed regulon 
xtract_replaced_links_from_regNet <- function(inferredLinks.df, regNet.pert){
  #inferredLinks.df <- inferredNet.retained.df
  # put the perturbed regNet on top of inferred links 
  combInt.df <- rbind(regNet.pert, inferredLinks.df) 
  ...
  inferredLinks_not_in_pertReg.df <- inferredLinks.df[dupStatus.n,]
  
  return(inferredLinks_not_in_pertReg.df)
} 
...
calPR4newLinks <- function(inferredRel.df, gtLinks.df, regNet.pert, miTSH.list){
  stats.df <- as.data.frame(matrix(nrow = length(miTSH.list), ncol = 6))
  rownames(stats.df) <- miTSH.list 
  colnames(stats.df) <- c('ppv', 'rCall', 'newlinks', 'gtLinks', 'prLinksCount', 'prLinks_not_in_pertReg')
  #i <- 1
  for(i in 1:length(miTSH.list)) {
    #print(i)
    inferredLinks.df = inferredRel.df[inferredRel.df$mi>=miTSH.list[i], c(1, 2, 4)]
    
    if (nrow(inferredLinks.df)==0){
      stats.df[i,] <- c(NA, NA, NA, NA, NA, NA)
      continue
    } 
    
    inferredLinks.df <- inferredLinks.df[, c(1, 2)] # retain only source and target
    inferredLinks.df <- format_source_target_nodes(inputNet.df=inferredLinks.df)
    
    #inferredNet.retained.df <- inferredNet.df[, c(1, 2)] 
    colnames(inferredLinks.df) <- colnames(gtLinks.df) 
    dupStatus <- duplicated(inferredLinks.df)
    sum(dupStatus)

    inferredLinks.retained.df <- inferredLinks.df[!dupStatus,] # remove duplicated interactions
    dim(inferredLinks.retained.df)
    #inferredLinks.retained.df <- inferredLinks.df

    # Find the links in inferred link set that are NOT in the perturbed regulon
    inferredLinks_not_in_pertReg.df <- xtract_replaced_links_from_regNet(inferredLinks.df=inferredLinks.retained.df, regNet.pert)
    combInt.df <- rbind(gtLinks.df, inferredLinks_not_in_pertReg.df)  
    dupStatus <- duplicated(combInt.df) 
    
    if(sum(dupStatus)>0) {
      rCall <- sum(dupStatus)/nrow(gtLinks.df)   
      #ppv <- sum(dupStatus)/nrow(inferredLinks.retained.df) 
      ppv <- sum(dupStatus)/nrow(inferredLinks_not_in_pertReg.df) 
    }else rCall <- ppv <- NA
    stats.df[i,] <- c(ppv, rCall, sum(dupStatus), nrow(gtLinks.df), nrow(inferredLinks.retained.df), nrow(inferredLinks_not_in_pertReg.df) )
  } 
  return(stats.df) 
}
