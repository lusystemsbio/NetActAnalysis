...
library("affy")
library("sRACIPE")
#setwd(dir = "/Users/gordid/Desktop/NetAct/EMT/")
...
library(fastmatch)
data("hDB")
DErslt = MicroDegs(eset = neweset, compList = compare_list, qval = 0.05)
head(DErslt$Overall$table)

output = TF_Selection(GSDB = hDB, DErslt = DErslt, minSize = 8, nperm = 10000, qval = 0.01, compList = compare_list, ntop = NULL, nameFile = "GSEArslt")

act.me <- TF_Activity(output$tfs, hDB, neweset, DErslt$Overall)
act.me$all_list
acts_mat = act.me$all_activities
Activity_heatmap(acts_mat, neweset)

tf_links = TF_Filter(acts_mat, hDB, miTh = .05, nbins = 8, corMethod = "spearman", DPI = T, nameFile = "EMT_network")
